...
namespace M_No_Death_Randomness
{
    ...
    internal static class DetourInjector
    {
        ...
        {
            if (!DoDetour(typeof(Pawn_HealthTracker), typeof(Main), "CheckForStateChange")) return false;
            if (!DoDetour(typeof(Pawn_HealthTracker), typeof(Main), "ShouldBeDowned")) return false;
            if (!DoDetour(typeof(Pawn_HealthTracker), typeof(Main), "ShouldBeDead")) return false;
